...
namespace gzip {
    ...
    public:
        ...
        {
           printf("size arg is too large to fit into unsigned int type");
           return false;
        }
#endif
            if (size > max_)
            {
                printf("size may use more memory than intended when decompressing");
                return false;
            }
...
#pragma GCC diagnostic pop
            ...
            deflate_s.next_in = const_cast<Bytef*>(data);
    ...
    public:
        ...
        int decompress(OutputType& output,
                        const Bytef* data,
                        std::size_t size) const
        {
            z_stream inflate_s;
...
#pragma GCC diagnostic pop
            inflate_s.next_in = const_cast<Bytef*>(data);
        ...
        if (size_64 > std::numeric_limits<unsigned int>::max())
        {
            ...
            printf("size arg is too large to fit into unsigned int type x2");
            return false;
        }
#endif
            ...
            do
            {
                ...
                {
                    inflateEnd(&inflate_s);
                    printf("size of output string will use more memory then intended when decompressing");
                    return false;
                }
    ...
    inline std::string decompress(const char* data, std::size_t size)
    {
        Decompressor decomp;
        std::string output;
        int ret = decomp.decompress(output, (Bytef *)data, size);
        if(ret == 0)
            output = data;
        return output;
    }
                                ...
                                int level = Z_DEFAULT_COMPRESSION)
    {
        ...
        std::string output;
        int ret = comp.compress(output, (Bytef *)data, size);
        if(ret == 0)
            output = data;
        return output;
    }
