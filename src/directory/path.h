...
public:
    /// \brief Path types.
    enum PathType {
        path_type_file,      ///Native file system path. (On windows, '/' are turned into '\'. On linux, '\' are turned into '/')
        path_type_virtual    ///Path using slashes ('/').
    };
    ...
    /// Example #1:
    /// \code
    /// std::string str = Path::make_absolute("ABD/DEF/", "GFX/Filename.txt", Path::path_type_file);
    ...
    /// Example #2:
    /// \code
    /// std::string str = Path::make_absolute("/ABD/DEF/", "../Filename.txt", Path::path_type_file);
    ...
    /// Example #3:
    /// \code
    /// std::string str = Path::make_absolute("/ABD/DEF/", "../Filename.txt", Path::path_type_virtual);
    /// On Linux: str = "/ABD/Filename.txt"
    /// On Windows: str = "/ABD/Filename.txt"
    /// \endcode
    static std::string make_absolute(const std::string &base_path, const std::string &relative_path,
    ...
    /// Example #1 (Windows):
    /// \code
    /// std::string str = Path::make_relative("ABD/DEF/", "c:\user\ABD\Filename.txt", Path::path_type_file);
    /// On Windows: str = "..\Filename.txt"
    /// \endcode
    static std::string make_relative(const std::string &base_path, const std::string &absolute_path,
                                     PathType path_type = path_type_file);
    ...
    ///
    /// If (path_type == path_type_virtual) or not using windows, then this function returns the path (excluding
    /// filename)
    ...
    static std::string get_fullname(const std::string &fullpath, const std::string &filename,
                                    PathType path_type = path_type_file);
    ...
    /// \return The full name
    static std::string get_fullname(const std::string &fullpath, const std::string &filename,
                                    const std::string &extension, PathType path_type = path_type_file);
    ...
    /// \param: path_type = The path type
    /// \return The full name
    static std::string get_fullname(const std::string &location, const std::string &basepath,
