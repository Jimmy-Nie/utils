...
namespace jm
{
		...
		: handle(invalid_handle), peeked_data(0)
	{
		...
		if (!result)
			throw Exception(fmt::format("IODeviceProvider_File::IODeviceProvider_File(): Unable to open file '{}'", a_filename));
	}
	...
	size_t IODeviceProvider_File::read(void *buffer, size_t size, bool read_all)
	{
		...
		{
			size_t peek_amount = std::min(size, peeked_data.get_size());
