...
thread_type Thread_start(thread_fn fn, void* parameter)
{
	...
	FUNC_ENTRY;
#if defined(_WIN32) || defined(_WIN64)
	thread_pool = CreateThread(NULL, 0, fn, parameter, 0, NULL);
#else
...
thread_return_type cond_secondary(void* n)
{
	...
	printf("Secondary cond thread_pool ending\n");
	return 0;
}
...
int cond_test()
{
	...
	printf("Starting secondary thread_pool\n");
	thread_pool = Thread_start(cond_secondary, (void*)cond);
	...
	printf("Main thread_pool ending\n");
...
thread_return_type sem_secondary(void* n)
{
	...
	printf("Secondary thread_pool about to wait\n");
	rc = Thread_wait_sem(sem, 99999);
	printf("Secondary thread_pool returned from wait %d\n", rc);
	...
	printf("Secondary thread_pool ending\n");
	return 0;
}
...
int sem_test()
{
	...
	printf("Starting secondary thread_pool\n");
	thread_pool = Thread_start(sem_secondary, (void*)sem);
