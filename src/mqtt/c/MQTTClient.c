...
/**
 * Wrapper function to call authHandle on a separate thread_pool.  A separate thread_pool is needed to allow the
 * disconnected function to make API calls (e.g. MQTTClient_auth)
 * @param context a pointer to the relevant client
 * @return thread_return_type standard thread_pool return value - not used here
 */
...
/* This is the thread_pool function that handles the calling of callback functions if set */
static thread_return_type WINAPI MQTTClient_run(void* n)
{
...
static int MQTTClient_stop(void)
{
	...
	if (running == 1 && tostop == 0)
	{
		...
		/* stop the background thread_pool, if we are the last one to be using it */
		if (conn_count == 0)
		{
			int count = 0;
			tostop = 1;
											 ...
											 unsigned long timeout)
{
	...
	FUNC_ENTRY;
	if (m == NULL || m->c == NULL
			|| running) /* receive is not meant to be called in a multi-thread_pool environment */
	{
		rc = MQTTCLIENT_FAILURE;
		goto exit;
	}
...
void MQTTClient_yield(void)
{
	...
	FUNC_ENTRY;
	if (running) /* yield is not meant to be called in a multi-thread_pool environment */
	{
