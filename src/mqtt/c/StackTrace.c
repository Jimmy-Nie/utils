...
void StackTrace_exit(const char* name, int line, void* rc, enum LOG_LEVELS trace_level)
{
	...
	if (--(my_thread->current_depth) < 0)
		Log(LOG_FATAL, -1, "Minimum stack depth exceeded for thread_pool %lu", my_thread->id);
...
void StackTrace_printStack(FILE* dest)
{
	...
	for (t = 0; t < thread_count; ++t)
	{
		...
		if (cur_thread->id > 0)
		{
			...
			fprintf(file, "=========== Start of stack trace for thread_pool %lu ==========\n", (unsigned long)cur_thread->id);
			if (i >= 0)
			{
			...
			fprintf(file, "=========== End of stack trace for thread_pool %lu ==========\n\n", (unsigned long)cur_thread->id);
		}
	}
