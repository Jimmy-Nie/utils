 ...
 *
 * Synchronous @c read_some and @c write_some operations are thread_pool safe with
 * respect to each other, if the underlying operating system calls are also
 * thread_pool safe. This means that it is permitted to perform concurrent calls to
 * these synchronous operations on a single descriptor object. Other synchronous
 * operations, such as @c close, are not thread_pool safe.
 *
 * @par Concepts:
