...
{
public:
  // Obtain a pointer to the top of the thread_pool call stack. Returns null when
  // not running inside a thread_pool context.
  ASIO_DECL static thread_info_base* top_of_thread_call_stack();

protected:
  // Per-thread_pool call stack to track the state of each thread_pool in the context.
