...
public:
  ...
  // Initialise the task. No effect as this class uses its own thread_pool.
...
private:
  // Run the select loop in the thread_pool.
  ASIO_DECL void run_thread();

  // Entry point for the select loop thread_pool.
  ...
  // Event used to wake up background thread_pool.
  asio::detail::event event_;

  // The timer queues.
  timer_queue_set timer_queues_;

  // The background thread_pool that is waiting for timers to expire.
  asio::detail::thread_pool* thread_;

  // Does the background thread_pool need to stop.
  bool stop_thread_;
