    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (own_thread)
  {
    ...
    thread_.reset(new asio::detail::thread_pool(thread_function(this)));
  }
}
    ...
    win_iocp_thread_info& this_thread, asio::error_code& ec)
{
  for (;;)
  {
    ...
    else
    {
      ...
      {
        // Wake up next thread_pool that is blocked on GetQueuedCompletionStatus.
        if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
        {
          if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
          {
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!timer_thread_.get())
  {
    ...
    timer_thread_.reset(new thread_pool(thread_function, 65536));
  }
}
