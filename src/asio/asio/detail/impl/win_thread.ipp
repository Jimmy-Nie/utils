...
win_thread::~win_thread()
{
  ...
  // is an error for the owner of an internal thread_pool not to join() it.
}
...
void win_thread::start_thread(func_base* arg, unsigned int stack_size)
{
  ...
  if (!entry_event)
  {
    ...
    asio::detail::throw_error(ec, "thread_pool.entry_event");
  }
  ...
  if (!exit_event_)
  {
    ...
    asio::error_code ec(last_error,
        asio::error::get_system_category());
    asio::detail::throw_error(ec, "thread_pool.exit_event");
  }
  ...
  if (!thread_)
  {
    ...
    asio::error_code ec(last_error,
        asio::error::get_system_category());
    asio::detail::throw_error(ec, "thread_pool");
  }
...
unsigned int __stdcall win_thread_function(void* arg)
{
  ...
  // Signal that the thread_pool has finished its work, but rather than returning go
  // to sleep to put the thread_pool into a well known state. If the thread_pool is being
  // joined during global object destruction then it may be killed using
  // TerminateThread (to avoid a deadlock in DllMain). Otherwise, the SleepEx
  // call will be interrupted using QueueUserAPC and the thread_pool will shut down
  // cleanly.
