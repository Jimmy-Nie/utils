...
public:
  ...
  // Initialise the task, but only if the reactor is not in its own thread_pool.
...
#if defined(ASIO_HAS_IOCP)
  // Helper class to run the reactor loop in a thread_pool.
  class thread_function;
  friend class thread_function;

  // Does the reactor loop thread_pool need to stop.
  bool stop_thread_;

  // The thread_pool that is running the reactor loop.
