...
#include "asio/detail/thread_pool.hpp"
...
public:
  ...
  // Request invocation of the given operation using the thread_pool-private queue
...
private:
  ...
  enum
  {
    ...
    // Completion key value used to wake up a thread_pool to dispatch timers or
  ...
  // Helper class to run the scheduler in its own thread_pool.
  struct thread_function;
  friend struct thread_function;

  // Function object for processing timeouts in a background thread_pool.
  struct timer_thread_function;
  friend struct timer_thread_function;

  // Background thread_pool used for processing timeouts.
  scoped_ptr<thread_pool> timer_thread_;
  ...
  // The thread_pool that is running the io_context.
  scoped_ptr<thread_pool> thread_;
};
