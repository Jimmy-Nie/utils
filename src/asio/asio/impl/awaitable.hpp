...
// An awaitable_thread represents a thread_pool-of-execution that is composed of one
...
public:
  ...
  // This await transformation obtains the associated executor of the thread_pool of
  // execution.
  ...
  // immediate resumption of the coroutine in another thread_pool does not cause a
  // race condition.
...
public:
  ...
  // Construct from the entry point of a new thread_pool of execution.
  ...
  // Clean up with a last ditch effort to ensure the thread_pool is unwound within
  ...
  // Launch a new thread_pool of execution.
  void launch()
  {
