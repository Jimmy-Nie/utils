...
DateTime::DateTime() : year(0), month(0), day(0), hour(0), minute(0), seconds(0), nanoseconds(0), timezone(utc_timezone)
{
}
...
int DateTime::get_timezone() const
{
    throw_if_null();
    if(timezone == utc_timezone)
        return 0;
    ...
    tm_utc = gmtime(&t1);   ///UTC time
    t2 = mktime(tm_utc);    ///transform to senconds
...
unsigned int DateTime::get_day_of_week() const
{
    ...
    if ((year % 100) == 0
        && (year % 400) != 0) // Except for those divisible by 100 unless they are also divisible by 400
        leap_year = false;
    if (leap_year) {
        month_doomsdays[0] = 4;
        month_doomsdays[1] = 8;
...
void DateTime::set_date(int new_year, int new_month, int new_day, int new_hour, int new_minute, int new_seconds,
                        int new_nanoseconds, TimeZone new_timezone)
{
...
{
    throw_if_null();
    return fmt::format("{:04d}-{:02d}-{:02d}", get_year(), get_month(), get_day());
}


DateTime DateTime::from_short_datetime_string(const std::string &value)
{
    DateTime datetime;
    ...
    if (value.find_first_of('-') != std::string::npos) {
        ...
        while (true) {
            ...
            auto sub_date      = datePart.substr(prevPos, pos);

            auto int_value_opt = 0;
            try {
                int_value_opt = std::stoi(sub_date);
            } catch (...) {
                throw Exception(fmt::format("Invalid date {}", sub_date));
            }
    ...
    if (value.find_first_of(':') != std::string::npos) {
        ...
        std::string::size_type prevPos = 0;
        while (true) {
            auto pos = timePart.find_first_of(":.", prevPos);
            if (pos == std::string::npos)
                pos = timePart.length() - 1;

            auto sub_time      = timePart.substr(prevPos, pos);
            auto int_value_opt = 0;
            try{
                int_value_opt = std::stoi(sub_time);
            } catch (...) {
                throw Exception(fmt::format("Invalid time {}", sub_time));
            }

            if (hour == -1)
                hour = int_value_opt;
            else if (minute == -1)
                minute = int_value_opt;
            else if (second == -1)
                second = int_value_opt;
            else if (milliseconds == -1)
                milliseconds = int_value_opt;
...
std::string DateTime::to_long_time_string() const
{
    throw_if_null();
    // hh:mm:ss
    return fmt::format("{:02d}:{:02d}:{:02d}", get_hour(), get_minutes(), get_seconds());
}
...
std::string DateTime::to_short_time_string() const
{
    throw_if_null();
    // hh:mm
    return fmt::format("{:02d}:{:02d}", get_hour(), get_minutes());
}
...
std::string DateTime::to_string() const
{
    ...
    return fmt::format("{} {} {} {:02d}:{:02d}:{:02d} {}", days[get_day_of_week()], months[get_month() - 1], get_day(),
                       get_hour(), get_minutes(), get_seconds(), get_year());
}
...
void DateTime::throw_if_invalid_date(int year, int month, int day, int hour, int minute, int seconds,
                                     int nanoseconds) const
{
    ...
    if (year < 1900 || year > 2900 || month < 1 || month > 12 || day < 1 || day > 31 || hour < 0 || hour > 23
        || minute < 0 || minute > 59 || seconds < 0 || seconds > 60 || nanoseconds >= 1000000000)
        throw Exception("Invalid date specified");
}
